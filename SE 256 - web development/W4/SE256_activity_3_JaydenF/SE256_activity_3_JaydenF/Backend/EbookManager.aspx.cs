...
namespace SE256_activity_3_JaydenF.Backend
{
    public partial class EbookManager : System.Web.UI.Page
    {
        ...
        {
            BookSearchResult bookSearchResultInstance = new BookSearchResult();
            bookSearchResultInstance.DeleteBook(int.Parse(Request.QueryString["Id"]));

            feedback_label.Text = "Book deleted successfully!";

            // Clear screen if deleted properly
            if (feedback_label.Text == "Book deleted successfully!")
            {
                Response.Redirect("~/Backend/EbookManager.aspx");
            }
        }
